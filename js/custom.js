...
function genomes (s) {
  if (s) {
    ...
    var a = s.split(' ');
    for (i = 0; i < a.length; i++) {
	out += '<a href="../data/genomes/' + a[i] + '" download>' + 
...
var columnsDef = [
  ...
  },
  {
    ...
    "render": function (data, type, row, meta) {
        if(data) {
          return '<a href="../data/phenotypes/' + data + '" download>Binary</a> <span class="glyphicon glyphicon-download" aria-hidden="true"></span>'
        }
        else { return '' }
    }
  },
